/* prayer-engine.js - BIK Kosovo primary + Aladhan/Diyanet APIs as alternatives
 *
 * BIK strategy:
 *  1. Fetch official JSON from drilonjaha/kohet-e-namazit-kosove-json on first load
 *  2. Cache forever (same data every year per BIK methodology)
 *  3. If offline and no cache: compute locally with BIK parameters
 *     (Fajr 18 deg, Isha 17 deg, Hanafi school, +6 min Temkin)
 */
(function(global){
'use strict';
...
const SOURCES={
  bik:{label:'BIK',primary:true,timezone:'Europe/Belgrade'},
...
function parseTimeToDate(base,hhmm){
  ...
  if(!m)return null;
  const dt=new Date(base);dt.setHours(parseInt(m[1],10),parseInt(m[2],10),0,0);return dt;
}
function formatTime(d){if(!d)return'--:--';return pad2(d.getHours())+':'+pad2(d.getMinutes())}
function formatCountdown(ms){if(!isFinite(ms)||ms<0)ms=0;const t=Math.floor(ms/1000),h=Math.floor(t/3600),m=Math.floor((t%3600)/60),s=t%60;if(h>0)return h+':'+pad2(m)+':'+pad2(s);return pad2(m)+':'+pad2(s)}

/* Astronomical calculation - matches BIK methodology */
...
function sunPosition(jd){const D=jd-2451545.0;const g=toRad((357.529+0.98560028*D)%360);const q=(280.459+0.98564736*D)%360;const L=toRad((q+1.915*Math.sin(g)+0.020*Math.sin(2*g))%360);const e=toRad(23.439-0.00000036*D);const RA=toDeg(Math.atan2(Math.cos(e)*Math.sin(L),Math.cos(L)))/15;const decl=toDeg(Math.asin(Math.sin(e)*Math.sin(L)));const EqT=q/15-normHr(RA);return{decl:decl,EqT:EqT}}
function computeTime(jd,lat,lng,angle,dir,baseHrs){let t=baseHrs/24;for(let i=0;i<3;i++){const sp=sunPosition(jd+t);const cosH=(-Math.sin(toRad(angle))-Math.sin(toRad(lat))*Math.sin(toRad(sp.decl)))/(Math.cos(toRad(lat))*Math.cos(toRad(sp.decl)));if(cosH>1||cosH<-1)return null;const H=toDeg(Math.acos(cosH))/15;const noon=12-sp.EqT-lng/15;t=(dir==='before'?noon-H:noon+H)/24}return normHr(t*24)}
function asrHr(jd,lat,lng,factor){let t=13/24;for(let i=0;i<3;i++){const sp=sunPosition(jd+t);const A=factor+Math.tan(toRad(Math.abs(lat-sp.decl)));const ang=-toDeg(Math.atan(1/A));const cosH=(-Math.sin(toRad(ang))-Math.sin(toRad(lat))*Math.sin(toRad(sp.decl)))/(Math.cos(toRad(lat))*Math.cos(toRad(sp.decl)));if(cosH>1||cosH<-1)return null;const H=toDeg(Math.acos(cosH))/15;const noon=12-sp.EqT-lng/15;t=(noon+H)/24}return normHr(t*24)}
function hoursToHHMM(hrs,tz,addMin){if(hrs==null)return null;addMin=addMin||0;const h=normHr(hrs+tz)+addMin/60;let H=Math.floor(h),M=Math.round((h-H)*60);if(M===60){M=0;H=(H+1)%24}return pad2(H)+':'+pad2(M)}
function computeLocalBIK(date,lat,lng,school){
  ...
  return{Fajr:hoursToHHMM(fajrH,tz,TEMKIN),Sunrise:hoursToHHMM(sunriseH,tz),Dhuhr:hoursToHHMM(dhuhrH,tz,TEMKIN),Asr:hoursToHHMM(asrH,tz,TEMKIN),Maghrib:hoursToHHMM(maghribH,tz,TEMKIN),Isha:hoursToHHMM(ishaH,tz,TEMKIN)};
}
...
async function loadBIKData(){
  ...
  try{
    ...
    const res=await fetch(BIK_JSON_URL,{signal:ctrl.signal});
    clearTimeout(tm);
...
function bikLookup(data,date){
  if(!data||!data.prayer_times)return null;
  const m=MONTH_EN[date.getMonth()];
  const arr=data.prayer_times[m];
  if(!arr)return null;
  const e=arr.find(function(d){return d.day===date.getDate()});
  if(!e)return null;
  return{Fajr:String(e.fajr).replace('.',':'),Sunrise:String(e.sunrise).replace('.',':'),Dhuhr:String(e.dhuhr).replace('.',':'),Asr:String(e.asr).replace('.',':'),Maghrib:String(e.maghrib).replace('.',':'),Isha:String(e.isha).replace('.',':')};
}
function applyCityOffset(times,offset){
  if(!offset||!times)return times;
  const out={};
  for(const k in times){
    const v=times[k];if(!v){out[k]=v;continue}
    const p=v.split(':');if(p.length<2){out[k]=v;continue}
    const t=parseInt(p[0],10)*60+parseInt(p[1],10)+offset;
    const n=((t%1440)+1440)%1440;
    out[k]=pad2(Math.floor(n/60))+':'+pad2(n%60);
  }
  return out;
}

/* Aladhan API (used for both Aladhan and Diyanet sources) */
...
async function getForDate(date){
  ...
  let times=null,networkUsed=false,verified=false;
  if(source==='bik'){
    const data=await loadBIKData();
    if(data){times=bikLookup(data,date);if(times){verified=true;times=applyCityOffset(times,prefs.location.cityOffset||0)}}
    if(!times){times=computeLocalBIK(date,prefs.location.lat,prefs.location.lng,1);times=applyCityOffset(times,prefs.location.cityOffset||0)}
  }else{
    if(navigator.onLine){times=await fetchFromAladhan(date,prefs.location.lat,prefs.location.lng,cfg.method,cfg.school,prefs.location.tz||cfg.timezone);if(times)networkUsed=true}
    if(!times){const c=await global.XR_Storage.getPrayerDay(id);if(c&&c.times)times=c.times}
    if(!times)times=computeLocalBIK(date,prefs.location.lat,prefs.location.lng,cfg.school);
  }
  const hijri=gregorianToHijri(date);
  const rec={id:id,date:ymd(date),source:source,times:times,hijri:hijri,hijriSq:formatHijriSq(hijri),location:prefs.location,verified:verified,computed:!networkUsed&&!verified};
  if(networkUsed||verified){try{await global.XR_Storage.savePrayerDay(rec)}catch(e){}try{await global.XR_Storage.setPrefs({lastSync:Date.now()})}catch(e){}}
  return rec;
}
...
function currentAndNext(rec,now){
  ...
  for(let i=0;i<entries.length;i++){if(now<entries[i].date){nxt=entries[i];cur=i>0?entries[i-1]:Object.assign({},entries[entries.length-1],{previousDay:true});break}}
