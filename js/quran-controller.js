...
function buildReciterSelect(){
  const sel=Q.els.qfReciterSel;if(!sel)return;
  const R=global.XR_Quran.RECITERS;
  sel.innerHTML=Object.keys(R).map(function(k){return '<option value="'+k+'">'+esc(R[k].label)+'</option>'}).join('');
  sel.value=Q.prefs.reciter||global.XR_Quran.DEFAULT_RECITER;
}
...
function wirePlayer(){
  ...
  Q.els.qfReciterSel.addEventListener('change',function(e){
    Q.prefs.reciter=e.target.value;savePrefs({reciter:e.target.value});
    const wasPlaying=!a.paused;const t=a.currentTime;
    if(Q.curSurah){Q.audio.dataset.reciter='__';prepareTrack(Q.curSurah,false);a.addEventListener('loadedmetadata',function once(){a.currentTime=Math.min(t,a.duration||t);if(wasPlaying)a.play().catch(function(){});a.removeEventListener('loadedmetadata',once)})}
  });
  // sleep timer
