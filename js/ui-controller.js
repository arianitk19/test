...
async function boot(){
  ...
  global.XR_Prayer.setSource(state.prefs.prayerSource);
  applyTheme(state.prefs.theme);
  wireNav();wireSettings();wireGalleryViewer();wireInstall();wireConnectivity();wireImamVideo();
  if(global.XR_QuranUI){try{await global.XR_QuranUI.boot()}catch(e){console.warn('Quran UI boot',e)}}
  // Preload BIK data in background
  ...
  state.timers.refresh=setInterval(async function(){await refreshPrayerData();refreshHomeView();refreshPrayerView()},60000);
}
