...
(function(global){
'use strict';
const DB='xhamia-ratkoc-db',V=2;
const SP='preferences',SR='prayerCache',SN='notifications',SM='meta';
...
async function put(s,v){const t=await tx(s,'readwrite');if(!t){lsS(s,v.key||v.id,v);return v}return new Promise(function(res){const r=t.put(v);r.onsuccess=function(){res(v)};r.onerror=function(){lsS(s,v.key||v.id,v);res(v)}})}
async function get(s,k){const t=await tx(s,'readonly');if(!t)return lsG(s,k);return new Promise(function(res){const r=t.get(k);r.onsuccess=function(){res(r.result||null)};r.onerror=function(){res(lsG(s,k))}})}
async function getAll(s){const t=await tx(s,'readonly');if(!t)return[];return new Promise(function(res){const r=t.getAll();r.onsuccess=function(){res(r.result||[])};r.onerror=function(){res([])}})}
async function del(s,k){const t=await tx(s,'readwrite');if(!t){lsD(s,k);return}return new Promise(function(res){const r=t.delete(k);r.onsuccess=function(){res()};r.onerror=function(){lsD(s,k);res()}})}
async function clr(s){const t=await tx(s,'readwrite');if(!t)return;return new Promise(function(res){const r=t.clear();r.onsuccess=function(){res()};r.onerror=function(){res()}})}

async function getPrefs(){if(mem.prefs)return Object.assign({},DEFAULTS,mem.prefs);const rec=await get(SP,'main');const p=rec&&rec.value?Object.assign({},DEFAULTS,rec.value):Object.assign({},DEFAULTS);mem.prefs=p;return p}
async function setPrefs(partial){const cur=await getPrefs();const nxt=Object.assign({},cur,partial);if(partial.location)nxt.location=Object.assign({},cur.location,partial.location);mem.prefs=nxt;await put(SP,{key:'main',value:nxt});document.dispatchEvent(new CustomEvent('prefs:changed',{detail:nxt}));return nxt}
...
async function clearAll(){await clr(SP);await clr(SR);await clr(SN);await clr(SM);mem.prefs=null;try{Object.keys(localStorage).filter(function(k){return k.indexOf('xr.')===0}).forEach(function(k){localStorage.removeItem(k)})}catch(e){}}

global.XR_Storage={DEFAULTS:DEFAULTS,getPrefs:getPrefs,setPrefs:setPrefs,resetPrefs:resetPrefs,savePrayerDay:savePrayerDay,getPrayerDay:getPrayerDay,getAllPrayers:getAllPrayers,purgeOldPrayers:purgeOldPrayers,saveNotification:saveNotification,getPendingNotifications:getPendingNotifications,markNotificationFired:markNotificationFired,clearAllNotifications:clearAllNotifications,purgeFiredNotifications:purgeFiredNotifications,setMeta:setMeta,getMeta:getMeta,clearAll:clearAll};
})(window);
